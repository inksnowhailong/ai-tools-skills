---
name: tvs-pullread
description: 【已废弃】远程变更解读 —— 当用户说"看看远程改了什么、拉之前帮我看看、别人提交了啥、分析下这个 PR、远程有什么更新、这次合并安不安全"等远程变更解读类诉求时使用。作用：在业务/功能层面提炼远程 diff（不逐文件罗列改动），标注每个变更的意图与风险，提示潜在合并冲突；分析后询问是否合并、是否需要协助处理冲突。
disable-model-invocation: true
---

> **⚠ 本 skill 已废弃**（2026-09-21）。
> **无直接替代**。需要时：`git fetch` 后把 `base...branch` 的 diff 交给 Claude 读，或用 `/code-review <分支>`。
> 文件保留仅供查阅与回滚，不再维护；已关闭模型自动调用，仍可用 `/tvs-pullread` 手动唤起。
