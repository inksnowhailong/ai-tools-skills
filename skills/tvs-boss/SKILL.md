...
- `scripts/panel.mjs` —— 零依赖终端 TUI 面板：`node scripts/panel.mjs [--root <团队根>]`，终端里看 雷达/项目/守则/契约（有 active.md 时多「任务」屏）（**←→ 或 1~5 切屏；↑↓ 或 j/k 在当前屏滚动一行，PgUp/PgDn 翻页，g/G 跳首尾；q/Ctrl+C 退出**——横向键管 tab、纵向键管滚动）。**默认首屏「行动雷达」**：跨所有项目 + tvs-task 把"需 boss 动手的"按紧要度聚成一屏（◉ 未提交且停滞→别丢了 / ↑ 领先未合→开PR / ↓ 落后→rebase / ◔ 进行中任务停滞→推进?），顶部并入精简统计徽章（项目/在途分支/未提交），无事则 `✓ 一切安好`；全 git+文件派生（读 active.md 解析进行中任务）。雷达阈值（STALE_DAYS/AHEAD_PR/BEHIND_REBASE/TASK_STALE_DAYS）在 panel.mjs 顶部常量块一处可调。（任务停滞图标用宽度1的 ◔ 替 🕐 emoji，护对齐。）**展示原则：panel 只呈现 git 派生（总览/项目）+ 真实文件原文（守则/契约/任务），即时读就准；不呈现需 leader 人工同步的运行态（已移除靠 live-agents.json 的「团队/在岗」屏与「在岗 N」统计）。** 内容超高可纵向滚动、不截断，边线显示滚动指示；Tokyo Night 柔和 256-color 配色护眼、无色终端降级；统一符号（◆logo·⎇分支·◉N改·◌干净·↑↓领先落后·⊘N藏·▸在途·●仅"实时"）；总览有异常时多一行金黄 `⚠ 需关注`，项目屏带 stash 数与各在途分支最近活跃时间、项目间编号分隔线；圆角外框；fs 变即时刷 + 每 REFRESH_MS(默认4s) 后台静默重扫 git（异步不阻塞滚动、数据没变不重绘）；有 ~/.tasklog/active.md 时多出末屏「任务」，富 markdown（含 `[ ]/[x]`→◌/✓）渲染干净。`--root` 接 团队根 / `.tvs-boss` 目录 / 其下子目录 都能正确解析，不依赖 cwd。
