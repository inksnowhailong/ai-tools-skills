...
6. **完成语义**——功能分支 commit 跟验收走、push 跟闸口走（自动）；合并干线 / 向干线 push 必须 boss 确认。
...
10. **重任务波次并行**——四查判"重"先拆解成范围互斥子任务图，按依赖分波、波内并行 spawn，汇合审查；leader 回合数从棒数降到波数。
11. **work/ 运行态暂存区**——过程产物唯一落点 `.tvs-boss/work/<需求slug>/`，交付即清；`.tvs-boss/` 顶层白名单化。
