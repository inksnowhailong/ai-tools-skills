...
**改过 page.html 之后**：页面是所有标准面板共用的同一份源码，但**每块板是独立的 artifact**——要**逐块带 `url` 重发，发完当场 `--set` 重记指纹**。库里的数据不受影响。
...
> ⚠ **`pageStale` 为 true 有两种可能，别一上来就重发。**
> 一是真旧版；二是**上次发了但漏跑 `--set`**，页面其实是新的、只有指纹停在旧值。
> 两者 `--list` 里长得一模一样，实测遇到过后者。分辨与处理：
>
> ```
> 先 Artifact read 那块板（重发本来也必须先 read，否则 publish 被拒「未先查看最新版」）
>   ├─ live HTML 与本地 page.html 一致 → 假阳性：只跑 --set 重记指纹，不要 publish
>   │                                    （硬发会被拒「内容与已拒版本相同」，白跑两轮）
>   └─ 确有差异                        → 真旧版：带 url 重发，发完 --set
> ```
>
> 根因是发布与记指纹是两个动作，中间断得开——`Artifact` 发布这件事 `panel.mjs` 感知不到，
> 只能靠发完那一下自己跑 `--set`。所以上面那句"发完当场"不是客套。
