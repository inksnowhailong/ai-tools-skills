...
# tvs-init-memory-system：一次性部署团队级记忆系统 + codegraph 分工配合（v6）

> **v6 账本模型**：`.memory` 只存**不可推导的知识**——业务导航（术语↔入口，AI 领域理解与 codegraph 锚点）、决策日志（append-only，为什么）、红线与约定（违反即错）、墓碑（证伪认知防复发）。可推导内容（模块职责/业务流程/数据契约）一律不存：必然过期且 codegraph 能现场推导。不建模块档案目录树，总量软上限 32KB。条目带可信度分数（1~5，升分只靠用户信号）。
...
└── references/
    ...
    ├── migration-v6.md               ← 存量 v4/v5 升级 v6（分数/墓碑/SessionStart 注入）
...
5. 是否已存在维护子 Agent 或 `.memory/`？存在则先读取并询问：是覆盖重建，还是按「存量升级」节（v4 走 migration-v5 蒸馏后接 migration-v6；v5 直接 migration-v6）做升级。
...
读 `references/hook-install.md` 的注册格式段，按宿主格式注册（Cursor `hooks.json` / Claude `settings.json` / Codex `config.toml`+`hooks.json`）。已有配置一律**先读后合并**，不覆盖。`TOOL = claude-code` 时额外注册 `SessionStart` 事件（`--print-index` 索引注入），见 hook-install.md。

### 产物 4：.memory/ 骨架（v6 账本）

读 `references/memory-skeleton.md`，建立七个入库文件：`记忆索引.md`（含查询流水线）、`业务导航.md`、`决策日志.md`、`红线与约定.md`、`跨分支在研功能地图.md`（含机读锚点）、`待确认问题.md`、`墓碑.md`（append-only 证伪记录）；`NATIVE_MEMORY = false` 时额外创建 gitignored 的 `个人偏好.md`。**不建模块档案目录树**。gitignore 清单见该文件（`MEMORY_IGNORED = true` 时用整库忽略分支）。
...
- **v4（有 `模块档案/` 目录树）**：先按 `references/migration-v5.md` 做**蒸馏迁移**（抽出决策/红线/术语入口填进账本，可推导内容删除），再按 `references/migration-v6.md` 增补 v6 三件套。
- **v5（已是账本模型）**：直接按 `references/migration-v6.md` 增量升级（维护员/宪法更新、墓碑骨架、SessionStart 注入），不动已有正文。
...
摘要之后**问一句："要不要现在做首次记忆盘点？（约 10 分钟，把业务导航填上初版，AI 立刻就能用你的叫法干活）"**——**默认强烈建议做**：不盘点的账本是纯模板假账，示例占位（如 `core/spaces/`）会被后续 AI 当真事实引用，净价值为负（实测教训）。

- 同意 → 读 `references/initial-inventory.md` 执行。
- 拒绝 → 必须在 `记忆索引.md` 顶部（标题下第一行）插入未启用标记，防假账被当真：
  ```markdown
  > ⚠️ 记忆未启用：仅骨架，业务导航为空、示例占位未清。AI 不得引用本库内容当项目事实；
  > 显式说"做记忆盘点"完成启用后删除本行。
  ```

之后停止，不进入日常维护态。不输出 diff、不输出 changelog。
