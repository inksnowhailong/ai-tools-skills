...
**`tvs-domain`** — 领域知识引擎，让 AI 不犯业务错误

> 代码结构 AI 能自己查，业务规则只有人知道。`tvs-domain` 用带可信度分数（1~5）的结构化 markdown 记录项目的业务概念、规则与系统对接关系：写业务代码前先读对应领域文件，低分知识用前先问，用户信号升分，被否定的落墓碑防复发——业务错误犯一次就不再犯第二次。
