...
**`tvs-council`** — 开个议会，把一件事议透而不是被附和

> AI 最省力的回答是顺着你说。`tvs-council` 围绕一份草案迭代：「书记」持有草案，四个上下文隔离的席位——「思绪」找漏掉的路、「架构师」算代价、「穷查理」做事前验尸、「用户」问值不值——对草案逐条开 issue、互相反驳，「书记」逐条处置出新版；席位每轮新建、靠状态表记得自己说过什么，镜头是起点不是边界。分歧先由席位表态代裁，只有相左的才到你手上，而且必须用白话写、附选项后果。开场问你最多几轮，末轮「书记」打包全部未决项，你回一句"按推荐执行"就算拍板；拍板产物是最终草案加每条 issue 的决策记录。产品、编程、Agent skill、人生选择都能议。仅 Claude Code。
...
> 你有多个项目同时在推进，每次都要开新对话、重新说背景——这是在浪费你最宝贵的注意力。`tvs-boss` 把当前目录下所有 git 项目纳为一个团队，你只需要下需求，Leader 自动调度 dev/review/test 等角色 Agent 协同开工。进度走原生 Task 实时账，状态栏单行速览各项目 git 健康度与记忆欠账，push 和合主线前必停等你拍板。
...
**`tvs-task`** — 跨会话任务账本，与 Claude Code 内置 Task 融合

> 内置 Task UI 只活一个会话，`tvs-task` 补上持久层：任务→子项两层结构写入 `~/.tasklog/`，状态与内置 Task 同构（pending/in_progress/completed），进度/待验收/停滞从 git 现推不落盘。开会话自动注入在册任务摘要，点名开工即播种进内置 Task UI 实时滚动，会话结束 hook 按锚自动回收进度；任务关闭权永远在用户手里——自动化最多推到 ⏳待验收。看任务默认开一块常驻 Artifact 面板（一个项目一块，没板自动建，写一次库所有打开着的面板跟着变），只有宿主没有面板能力时才退回终端字符画。

---

**`tvs-init-memory-system`** — 让 AI 真正"记住"这个项目

> 每次开新对话都要重新介绍项目背景，是因为 AI 没有持久记忆。`tvs-init-memory-system` 为当前项目一次性部署结构化记忆体系：`.memory/` 存只有人才知道的业务契约，codegraph 存可以自动查的代码结构——分工明确，AI 每次都能快速召回上下文，不再靠你重复喂背景。条目带可信度分数（1~5，低分知识用前先问、用户信号升分），被否定的认知落墓碑防复发——业务错误犯一次就不再犯第二次。
...
**`tvs-auto`** — skill 路由器，帮你选对工具再开工

> skill 装多了，最贵的反而是"每次该用哪个"的判断。`/tvs-auto <需求>` 会分诊需求模糊度、从当前环境已加载的 skill 里初筛并深读候选，推荐单个 skill、先后衔接的 skill 链，或直言"不用 skill 直接干"，你确认后立即执行；`-f` 直通模式跳过确认，但需求太模糊时照样先反问。
...
| `visual-explain.md` | on | 图示优先——五类字符画各两版（对话版/可贴版）按去向选，表格与正文用短行结构，人看的 md 用 mermaid |
