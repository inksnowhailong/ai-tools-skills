...
> 内置 Task UI 只活一个会话，`tvs-task` 补上持久层：任务→子项两层结构写入 `~/.tasklog/`，状态与内置 Task 同构（pending/in_progress/completed），进度/待验收/停滞从 git 现推不落盘。开会话自动注入在册任务摘要，点名开工即播种进内置 Task UI 实时滚动，会话结束 hook 按锚自动回收进度；任务关闭权永远在用户手里——自动化最多推到 ⏳待验收。按需 `panel.mjs` 轻量终端面板看全量。
