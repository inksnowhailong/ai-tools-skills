...
> AI 最省力的回答是顺着你说。`tvs-council` 围绕一份草案迭代：「书记」持有草案，四个上下文隔离的席位——「思绪」找漏掉的路、「架构师」算代价、「穷查理」做事前验尸、「用户」问值不值——对草案逐条开 issue，「书记」逐条处置出新版；被驳回的 issue 只能撤回或升级到你手上裁决，分歧要么解决要么到你面前，不会原地打转。未决 issue 归零是收敛信号，拍板只由你说；拍板产物就是最终草案加每条 issue 的决策记录。产品、编程、Agent skill、人生选择都能议。仅 Claude Code。
